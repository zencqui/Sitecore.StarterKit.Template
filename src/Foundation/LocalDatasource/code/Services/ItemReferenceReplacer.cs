...
namespace HelixExample.Foundation.LocalDatasource.Services
{
    public class ItemReferenceReplacer
    {
        ...
        {
            item.Fields.ReadAll();
            var fields = (IEnumerable<Field>) item.Fields
                .Where(f => f.ID == FieldIDs.LayoutField || 
                    f.ID == FieldIDs.FinalLayoutField || 
                    !f.Name.StartsWith("__")).ToArray();

            var itemVersions = item.Versions.GetVersions(true);

            return fields.SelectMany(
                field => itemVersions,
                (field, itemVersion) => itemVersion.Fields[field.ID]);
        }
    }
